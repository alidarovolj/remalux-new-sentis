...
public class ARManagerInitializer2 : MonoBehaviour
{
    ...
    private void Start()
    {
        Log("Start() called.", ARManagerDebugFlags.Initialization);
        FindARComponents(); // Call again in Start ensure all components are found, esp if some are created late
        InitializeMaterials();
        ...
        else if (trackablesParentInstanceID_FromStart == 0) // Добавляем проверку, если xrOrigin или его TrackablesParent не были найдены
        {
            LogError("XROrigin или его TrackablesParent не назначен! Невозможно отслеживать родителя для плоскостей.", ARManagerDebugFlags.Initialization);
        }
        ...
        // Ensure ARMeshManager's layer is in the hitLayerMask if enabled
        if (arMeshManager != null)
        {
            Log($"Start: ARMeshManager found. Name: {arMeshManager.name}, Layer: {LayerMask.LayerToName(arMeshManager.gameObject.layer)}, Enabled: {arMeshManager.enabled}. Current hitLayerMask: {LayerMaskToString(hitLayerMask)}", ARManagerDebugFlags.Initialization);
            if (arMeshManager.enabled && arMeshManager.gameObject != null)
            {
                int arMeshLayer = arMeshManager.gameObject.layer;
                Log($"Start: Checking if ARMeshManager layer '{LayerMask.LayerToName(arMeshLayer)}' (Value: {1 << arMeshLayer}) is in hitLayerMask (Value: {hitLayerMask.value}). Mask: {LayerMaskToString(hitLayerMask)}", ARManagerDebugFlags.Initialization);
                if ((hitLayerMask.value & (1 << arMeshLayer)) == 0) // If not already in the mask
                {
                    hitLayerMask.value |= (1 << arMeshLayer);
                    Log($"Added ARMeshManager layer '{LayerMask.LayerToName(arMeshLayer)}' to hitLayerMask. New mask: {LayerMaskToString(hitLayerMask)}", ARManagerDebugFlags.Initialization);
                }
                else
                {
                    Log($"Start: ARMeshManager layer '{LayerMask.LayerToName(arMeshLayer)}' is already in hitLayerMask. Mask: {LayerMaskToString(hitLayerMask)}", ARManagerDebugFlags.Initialization);
                }
            }
            else
            {
                LogWarning("Start: ARMeshManager is found but either not enabled or its gameObject is null. Cannot add its layer to hitLayerMask.", ARManagerDebugFlags.Initialization);
            }
        }
        else
        {
            LogWarning("Start: ARMeshManager is NULL. Cannot add its layer to hitLayerMask.", ARManagerDebugFlags.Initialization);
        }
    }
    ...
    private void ConfigurePlane(ARPlane plane)
    {
        ...
        if (!generatedPlanes.Contains(plane.gameObject) && plane.subsumedBy == null)
        {
            generatedPlanes.Add(plane.gameObject);
            planeCreationTimes[plane.gameObject] = Time.time;
        }
    ...
    {
        Log("OnDisable called.", ARManagerDebugFlags.System);
        if (planeManager != null) planeManager.planesChanged -= OnPlanesChanged;
        if (wallSegmentation != null) wallSegmentation.OnSegmentationMaskUpdated -= OnSegmentationMaskUpdated;
        if (arCameraManager != null) arCameraManager.frameReceived -= OnARFrameReceived;

        // Unsubscribe from ARMeshManager events
        if (arMeshManager != null)
        {
            arMeshManager.meshesChanged -= OnMeshesChanged;
            Log("Unsubscribed from ARMeshManager.meshesChanged.", ARManagerDebugFlags.System);
        }
        Log("Unsubscribed from all relevant events.", ARManagerDebugFlags.System);
    }
